...
from archsnap.custom_types import (
    ColourVars,
    ConfigValues,
    MeshQueueItem,
    OutputVars,
    RenderResolutionVars,
    SizeVar,
    SizeVarsDict,
)
...
def button_state_validation() -> None:
    """Enable or disable the add file and the render buttons."""
    # Disable the add file button if any of the file paths are empty
    add_file_button["state"] = (
        tk.DISABLED
        if any(
            x.entry_text.get() == ""
            for x in cast("list[_NewInputMesh]", inputs_frame.winfo_children())
        )
        else tk.NORMAL
    )
    # Enable the render button if there are not no mesh entries,
    # and no mesh entries have errors, and no entries are empty,
    # and the add file button is enabled
    render_button["state"] = (
        tk.NORMAL
        if all(
            x.error_text.get() == "" and x.entry_text.get() != ""
            for x in cast("list[_NewInputMesh]", inputs_frame.winfo_children())
        )
        and add_file_button["state"]
        and len(inputs_frame.winfo_children()) > 0
        else tk.DISABLED
    )


def _handle_render() -> None:
    """Handle the multiprocessed rendering functionality."""
    # Set up the work queue for multiprocessing the rendering across various instances
    # of blender and populate it from all the entries in the inputs frame as a list of
    # parameters of shape: mesh file path, output directory, use separate directories
    # boolean, use eevee renderer boolean, render resolution,
    # calculated mesh scale factor, current scalebar tick size, mesh colour and index
    mesh_queue: list[MeshQueueItem] = [
        {
            "mesh_path": x.entry_text.get(),
            "output_path": output_frame.output_path.get(),
            "separate_output_directories": output_frame.separate_output_directories_vars[  # noqa: E501
                "current"
            ].get(),
            "use_eevee": output_frame.use_eevee_vars["current"].get(),
            "render_resolution": output_frame.render_resolution_vars["current"].get(),
        ...
        for idx, x in enumerate(
            cast("list[_NewInputMesh]", inputs_frame.winfo_children()),
        )
    ]
    ...
    with Pool(len(mesh_queue)) as pool:
        ...
        # Close the queue, letting the worker processes to complete their tasks,
        ...
        open_output = messagebox.askyesno(
            ...
            """Image generation was successfully completed!
            \n\nOpen the output directory?""",
        )
        # If they said yes to opening the output directory, do so
        if open_output:
            webbrowser.open(str(Path(output_frame.output_path.get()).absolute()))
...
class _ConfigMeshModal(tk.Toplevel):
    ...
    def __init__(
        ...
        size_vars: SizeVarsDict,
        mesh_colour_vars: ColourVars,
        mesh_scale_factor: tk.DoubleVar,
        ...
        # Set the modal title
        self.title(f"Render settings for {Path(entry_text.get()).absolute().name}")
        ...
        # Create the instance size variables from the provided parameters,
        # sent as an array of initial, current, and previous size dictionaries
        # of StringVars
        self.initial_sizes, self.current_sizes, self.previous_sizes = (
            size_vars["initial"],
            size_vars["current"],
            size_vars["previous"],
        )
        ...
        self.mesh_colour_vars, self.mesh_scale_factor, self.scalebar_scale_factor = (
            mesh_colour_vars,
            mesh_scale_factor,
            scalebar_scale_factor,
        )

        # Create the tk.StringVar to store the colour selection error text
        # as an instance variable
        self.colour_error_text = tk.StringVar()
        # Create a tk.StringVar for the value of the scalebar length to display
        # in the respective label
        self.total_scalebar_size = tk.StringVar(
            value=f"Total scalebar length (10 ticks): "
            f"{float(self.current_sizes['scalebar_tick'].get()) * 10:.3f} cm",
        )

        # Process the entry text to get the absolute file path
        file_path = Path(entry_text.get()).absolute()

        # If the file path was somehow made invalid before opening the modal, exit
        if not file_path.is_file():
            messagebox.showerror("Error", "File does not exist!")
            self.destroy()
        if file_path.suffix not in [".ply", ".obj", ".stl", ".dae"]:
            messagebox.showerror("Error", "File type not supported!")
            self.destroy()

        # Create the main frame of the modal and add it to the grid
        config_mesh_modal_main_frame = self._ConfigMeshModalMainFrame(self)
        config_mesh_modal_main_frame.grid(pady=10, padx=10)

    class _ConfigMeshModalMainFrame(ttk.Frame):
        colour_picker_entry: ttk.Entry

        def __init__(self, container: Any) -> None:  # noqa: ANN401, PLR0915
            ...
            # Create the label for the mesh dimensions editing section and
            # add it to the grid
            dimensions_label = ttk.Label(labels_frame, text="Mesh dimensions in cm:")
            dimensions_label.grid(column=0, row=0, sticky="w")

            # Create the label for the x-dimension editing and add it to the grid
            mesh_x_size_label = ttk.Label(options_frame, text="x:")
            mesh_x_size_label.grid(column=0, row=0, sticky="w", padx=(0, 5))
            # Create the entry widget for the x-dimension editing and add it to the grid
            mesh_x_size_entry = ttk.Entry(
                options_frame,
                textvariable=container.current_sizes["x"],
                justify="center",
            )
            mesh_x_size_entry.configure(width=12)
            mesh_x_size_entry.grid(column=1, row=0, sticky="", padx=(0, 5))
            # Bind the KeyRelease event of the x-dimension entry widget to
            # a lambda function that passes the event object and the dimension code
            # (x = 0) to the method to handle the mesh size changes
            mesh_x_size_entry.bind(
                "<KeyRelease>",
                lambda event: container.handle_mesh_size_change(event, 0),
            )

            # Create the label for the y-dimension editing and add it to the grid
            mesh_y_size_label = ttk.Label(options_frame, text="y:")
            mesh_y_size_label.grid(column=2, row=0, sticky="w", padx=(0, 5))
            # Create the entry widget for the y-dimension editing and add it to the grid
            mesh_y_size_entry = ttk.Entry(
                options_frame,
                textvariable=container.current_sizes["y"],
                justify="center",
            )
            mesh_y_size_entry.configure(width=12)
            mesh_y_size_entry.grid(column=3, row=0, sticky="", padx=(0, 5))
            # Bind the KeyRelease event of the y-dimension entry widget to
            # a lambda function that passes the event object and the dimension code
            # (y = 1) to the method to handle the mesh size changes
            mesh_y_size_entry.bind(
                "<KeyRelease>",
                lambda event: container.handle_mesh_size_change(event, 1),
            )

            # Create the label for the z-dimension editing and add it to the grid
            mesh_z_size_label = ttk.Label(options_frame, text="z:")
            mesh_z_size_label.grid(column=4, row=0, sticky="w", padx=(0, 5))
            # Create the entry widget for the z-dimension editing and add it to the grid
            mesh_z_size_entry = ttk.Entry(
                options_frame,
                textvariable=container.current_sizes["z"],
                justify="center",
            )
            mesh_z_size_entry.configure(width=12)
            mesh_z_size_entry.grid(column=5, row=0, sticky="", padx=(0, 50))
            # Bind the KeyRelease event of the z-dimension entry widget to
            # a lambda function that passes the event object and the dimension code
            # (z = 2) to the method to handle the mesh size changes
            mesh_z_size_entry.bind(
                "<KeyRelease>",
                lambda event: container.handle_mesh_size_change(event, 2),
            )

            # Create the label for the editing of the scalebar and add it to the grid
            scalebar_tick_scale_label = ttk.Label(
                labels_frame,
                text="Scalebar tick length in cm:",
            )
            scalebar_tick_scale_label.grid(column=0, row=1, sticky="w", pady=(10, 0))
            # Create the entry widget for the editing of the scalebar and
            # add it to the grid
            scalebar_tick_size_entry = ttk.Entry(
                options_frame,
                textvariable=container.current_sizes["scalebar_tick"],
                justify="center",
            )
            ...
            scalebar_tick_size_entry.bind(
                "<KeyRelease>",
                container.handle_scalebar_tick_size_change,
            )

            # Create the label widget to display the total scalebar length and
            # add it to the grid
            scalebar_total_size_label = ttk.Label(
                options_frame,
                textvariable=container.total_scalebar_size,
            ...
            colour_picker_label = ttk.Label(colour_picker_frame, text="Mesh colour:")
            colour_picker_label.grid(column=0, row=0)
            # Create the entry widget for the manual editing of the mesh colour
            # as an instance variable and add it to the grid
            self.colour_picker_entry = tk.Entry(
                colour_picker_frame,
                textvariable=container.mesh_colour_vars["current"],
                justify="center",
            )
            ...
            self.colour_picker_entry.bind(
                "<KeyRelease>",
                partial(container.handle_colour_change, self),
            )

            # Create the button for opening the colour picker dialog and
            # add it to the grid
            colour_picker_button = ttk.Button(
                colour_picker_frame,
                text="Select colour",
                command=partial(container.choose_colour, self),
            )
            colour_picker_button.grid(column=2, row=0, padx=(5, 0), sticky="w")

            # Create the label widget to display any colour picker errors
            # as an instance variable and add it to the grid to actually create it,
            # then hide it by default until there is an error
            self.colour_error_label = ttk.Label(
                colour_picker_frame,
                textvariable=container.colour_error_text,
                foreground="red",
            )
            self.colour_error_label.grid(column=0, row=1, columnspan=3, sticky="w")
            self.colour_error_label.grid_forget()

            # Run an handling of colour change of the default colour
            # obtained from the config
            container.handle_colour_change(self)
            ...
            initials_button = ttk.Button(
                buttons_frame,
                text="Initial values",
                command=container.handle_reset_to_initial_values,
            )
            ...
            cancel_button = ttk.Button(
                buttons_frame,
                text="Cancel",
                command=container.handle_cancel,
            )
            ...
            save_button = ttk.Button(
                buttons_frame,
                text="Save",
                command=container.handle_save,
            )
            save_button.grid(column=2, row=0)

    def handle_mesh_size_change(  # noqa: C901, PLR0911, PLR0912
        self,
        _: None,
        ...
        match dimension:
            ...
            case 0:
                try:
                    # If we change x, see if we can convert the respective
                    # StringVar to a float
            ...
            case 1:
                try:
                    # If we change y, see if we can convert the respective
                    # StringVar to a float
            ...
            case 2:
                try:
                    # If we change z, see if we can convert the respective
                    # StringVar to a float
                ...
                if float(current_sizes["z"].get()) != 0:
                    ...
                    self.mesh_scale_factor.set(
                        float(current_sizes["z"].get())
                        / float(initial_sizes["z"].get()),
                    )
                else:
                    # If the value is 0, exit without doing anything
                    return 416

        # Loop through all three dimensions in both the current and initial sizes
        # with an index
        for idx, (size, initial) in enumerate(
            zip(
                [current_sizes["x"], current_sizes["y"], current_sizes["z"]],
                [initial_sizes["x"], initial_sizes["y"], initial_sizes["z"]],
                strict=True,
            ),
        ):
            # If the index is not the dimension
            # (since there is no need to change the values along the dimension
            # the user just changed)
            if idx != dimension:
                # Calculate and set the new size along the other dimensions
                # based on the mesh scale factor
                new_size = float(initial.get()) * float(self.mesh_scale_factor.get())
                size.set(
                    str(f"{new_size:.4f}"),
                )
        return 200

    def handle_scalebar_tick_size_change(self, _: None) -> Literal[200, 416, 422]:
        """Handle the change of the scalebar tick size."""
        # TODO: Validate against mesh dimensions to prevent too large scale factors  # noqa: E501, FIX002, TD002, TD003
        ...
        if float(scalebar_tick_size) > 0:
            # Calculate and set the scalebar scale factor based on the current
            # and initial tick sizes
            self.scalebar_scale_factor.set(
                float(scalebar_tick_size)
                / float(self.initial_sizes["scalebar_tick"].get()),
            )
            # Call the method to set the value of the StringVar to display the
            # total scalebar size
    ...
    def handle_colour_change(self, main_frame: _ConfigMeshModalMainFrame) -> None:
        """Validate a colour input or selection.

        Also change the look of the entry widget to reflect the colour chosen.
        """
        # Local variables to improve readability
        colour = self.mesh_colour_vars["current"].get()
        config_mesh_modal_main_frame = main_frame
        ...
        if colour and colour[0] == "#":
            ...
            try:
                ...
                # Convert those values (in decimal) to HLS to check the validity of
                # the colour code
                ...
                # Depending on the luminosity, we change the text colour
                # to white or black
                if hls[1] < 0.5:  # noqa: PLR2004
    ...
    def choose_colour(self, main_frame: _ConfigMeshModalMainFrame) -> None:
        ...
        if colour_code[1]:
            ...
            self.handle_colour_change(main_frame)

    def _set_scale_factors(self) -> None:
        ...
        self.scalebar_scale_factor.set(
            float(self.current_sizes["scalebar_tick"].get())
            / float(self.initial_sizes["scalebar_tick"].get()),
        )

    def _set_total_scalebar_size(self) -> None:
        """Set the display text of the total scalebar length."""
        total_scalebar_size = (
            "Total scalebar length (10 ticks): "
            f"{float(self.current_sizes['scalebar_tick'].get()) * 10:.3f} cm"
        )
        self.total_scalebar_size.set(total_scalebar_size)

    def handle_reset_to_initial_values(self) -> Literal[200, 304]:
        """Set all values to initial values."""
        # Check if there were any changes made in the parameters compared to
        # the initial values
        changed = any(
            current.get() != initial.get() or previous.get() != initial.get()
            for [(_, current), (_, previous), (_, initial)] in zip(
                cast("ItemsView[str, tk.StringVar]", self.current_sizes.items()),
                cast("ItemsView[str, tk.StringVar]", self.previous_sizes.items()),
                cast("ItemsView[str, tk.StringVar]", self.initial_sizes.items()),
                strict=True,
            )
        )
        ...
        if changed:
            ...
            self.mesh_colour_vars["current"].set(self.mesh_colour_vars["initial"].get())
        ...
        messagebox.showinfo("Initial values", "Parameters set to their initial values.")
        return 200

    def handle_cancel(self) -> Literal[200, 304]:
        ...
        changed = any(
            ...
            for [(_, current), (_, previous)] in zip(
                cast("ItemsView[str, tk.StringVar]", self.current_sizes.items()),
                cast("ItemsView[str, tk.StringVar]", self.previous_sizes.items()),
                strict=True,
            )
    ...
    def handle_save(self) -> None:
        ...
        # Since the current values are stored in tk.StringVars,
        # there's no need to do anything, just set the 'previous'
        # set of parameters to the current values so the cancel button
        ...
        self.mesh_colour_vars["previous"].set(self.mesh_colour_vars["current"].get())
...
class _NewInputMesh(ttk.Frame):
    ...
    def __init__(self, file_path: str | None = None) -> None:
        ...
        # Set the previously set (i.e. before any current edits) size instance variables
        # as empty tk.StringVars
        ...
        # Set the tk.DoubleVars for the scaling factors of the mesh and scale bar as
        # instance variables
        # and set them to 1
        ...
        # Create the input widget for the file path as an instance variable and
        # add it to the grid
        ...
        self.configure_button = ttk.Button(
            ...
            command=partial(
                ...
                self.entry_text,
                {
                    "initial": self.initial_sizes,
                    "current": self.current_sizes,
                    "previous": self.previous_sizes,
                },
                self.mesh_colour_vars,
                self.mesh_scale_factor,
                self.scalebar_scale_factor,
            ),
        )
        ...
        remove_entry_button = ttk.Button(
            self,
            text="❌",
        ...
        # add it to the grid to actually create it, then hide it by default
        ...
        # Create the button to browse for a file and add it to the grid
        # (at column 2, before the configure button)
        browse_button = ttk.Button(self, text="Browse…", command=self._select_mesh)
        ...
        if file_path is not None:
            ...
            self.entry_text.set(str(Path(file_path).absolute()))
            # Set the view of the entry widget to see the end of the path
            ...
            # Enable the configure button (since the file path is validated
            # by the directory selection)
    ...
    def _clear_mesh_parameters(self) -> None:
        ...
        self.mesh_colour_vars["initial"].set(CONFIG_VALUES["default_object_colour"])
        self.mesh_colour_vars["current"].set(CONFIG_VALUES["default_object_colour"])
        self.mesh_colour_vars["previous"].set(CONFIG_VALUES["default_object_colour"])

    def _load_initial_parameter(self) -> None:
        """Load the initial params for the mesh and save them to the instance vars."""
        # Check if the initial size is not yet changed
        # and if the file path is valid
        # and if the file type is supported
        if (
            self.initial_sizes["x"].get() == ""
            and Path(self.entry_text.get()).absolute().is_file()
            and Path(self.entry_text.get()).suffix in [".ply", ".obj", ".stl", ".dae"]
        ):
            # Call the get_mesh_args function to calculate the mesh and scale dimensions
            # as saved in the file
            dim_returns, scalebar_tick_returns = get_mesh_args(
                mesh_path=Path(self.entry_text.get()).absolute(),
            )

            # If this was not possible, exit the method
            if dim_returns is None or scalebar_tick_returns is None:
                return
            ...
            self.initial_sizes["scalebar_tick"].set(f"{scalebar_tick_returns:.4f}")
    ...
    def _handle_manual_change(self, event: tk.Event) -> None:
        """Handle the manual change of the file path entry widget."""
        # First, clear the initial parameters, since we changed the file path
        # and thus file used
        self._clear_mesh_parameters()

        if not event.widget.get():
            # Add an error message, add the label to the grid,
            # and disable the configure button
            self.error_text.set("Empty file path!")
            self.error_label.grid(columnspan=3, sticky="w")
            self.configure_button["state"] = tk.DISABLED
        # If the file path is not valid or the file does not exist
        elif not Path(event.widget.get()).absolute().is_file():
            # Add an error message, add the label to the grid,
            # and disable the configure button
        ...
        elif Path(event.widget.get()).suffix not in [".ply", ".obj", ".stl", ".dae"]:
            # Add an error message, add the label to the grid,
            # and disable the configure button
        ...
        # Else (the file exists and is supported)
            ...
            # Set the error text to empty, hide the error label,
            # and enable the configure button
            self.error_text.set("")
            self.error_label.grid_forget()
            self.configure_button["state"] = tk.NORMAL
    ...
    def _select_mesh(self) -> Literal[200, 406]:
        ...
        # Perform validation for the render button state now with
...
def _select_dir() -> None:
    ...
    # Whether the last entry from the input frame is empty or not
    # (to avoid overwriting it)
    last_entry_empty: bool = True
    ...
    if dir_path != "":
        ...
        for file_path in Path(dir_path).glob("*"):
            ...
            if file_path.suffix in [".ply", ".obj", ".stl", ".dae"]:
                # Get the last input frame (empty by default before any
                # file has been selected)
                new_input_mesh_frame = cast(
                    "_NewInputMesh",
                    inputs_frame.winfo_children()[-1],
                )
                # If the last entry from the input frame is empty
                # and the entry text is not empty (i.e. the file path is not empty)
                if not last_entry_empty and new_input_mesh_frame.entry_text.get() != "":
                ...
                if last_entry_empty:
                    ...
                    new_input_mesh_frame.entry_text.set(str(file_path.absolute()))
                # If it is not empty (e.g. we already added files from this directory)
                else:
                    # Do not overwrite it, and start a new instance of the
                    # new input mesh frame
                    new_input_mesh_frame = _NewInputMesh(
                        file_path=str(file_path.absolute()),
                    )

                # Set the configure button state to enabled, since the file path is
                # ipso facto valid
                ...
                # Perform validation for the render button state now with the
                # newly added file
                button_state_validation()
...
class _AddMeshButtonsFrame(ttk.Frame):
    ...
    def __init__(self, container: ttk.Frame) -> None:
        ...
        global add_file_button  # noqa: PLW0603
        # Create the button to add a single mesh and add it to the grid
        add_file_button = ttk.Button(self, text="\u2795 File", command=_NewInputMesh)
        ...
        add_dir_button = ttk.Button(self, text="\u2795 Folder", command=_select_dir)
        add_dir_button.grid(column=1, row=0)


def _select_output(entry: tk.Entry) -> None:
    """Select the output directory for the renders."""
    # Use the askdirectory function to open a directory selection dialog
    output_path = askdirectory(
        initialdir=f"{Path(output_frame.output_path.get()).parent!s}",
    )

    # If a directory was selected
    if output_path != "":
        # Set the tk.StringVar of the output directory to the selected directory
        output_frame.output_path.set(str(Path(output_path).absolute()))

        # Move the view of the entry widget to the end to see the end of the path,
        # the most important part
        entry.xview("end")
...
class _ConfigOutputModal(tk.Toplevel):
    ...
    def __init__(self) -> None:
        ...
        # Option to select if each mesh's output should be saved
        # in a separate directoryor not
        #
        # Create the label for the option and add it to the grid
        separate_dirs_label = ttk.Label(
            labels_frame,
            text="Save each mesh's output in a separate directory based on filename?",
        )
        ...
        separate_dirs_checkbox = ttk.Checkbutton(
            options_frame,
            variable=output_frame.separate_output_directories_vars["current"],
            onvalue=1,
            offvalue=0,
        )
        ...
        # Create the label for the option and add it to the grid
        use_eevee_label = ttk.Label(labels_frame, text="Use faster EEVEE renderer?")
        use_eevee_label.grid(column=0, row=1, sticky="w")
        # Create the checkbox for the option and add it to the grid
        use_eevee_checkbox = ttk.Checkbutton(
            options_frame,
            variable=output_frame.use_eevee_vars["current"],
            onvalue=1,
            offvalue=0,
        )
        ...
        render_resolution_label = ttk.Label(
            labels_frame,
            text="Render resolution in px (height and width):",
        )
        ...
        render_resolution_entry = ttk.Entry(
            options_frame,
            textvariable=output_frame.render_resolution_vars["current"],
            justify="center",
        )
        ...
        # Create the frame for the modal buttons and add it to the grid
        buttons_frame = ttk.Frame(self)
        buttons_frame.grid(column=0, row=1, pady=(0, 10), padx=10, sticky="nsew")
        ...
        defaults_button = ttk.Button(
            ...
            command=self._handle_factory_defaults,
        )
    ...
    def _handle_factory_defaults(self) -> Literal[200, 304]:
        ...
        changed = any(
            var["current"].get() != var["default"].get()
            or var["previous"].get() != var["default"].get()
            for var in (
                separate_output_directories_vars,
                use_eevee_vars,
                render_resolution_vars,
            )
        )
        ...
        if changed:
            ...
            confirm = messagebox.askyesno(
                ...
                "Are you sure you want to reset all parameters"
            ...
            render_resolution_vars["current"].set(DEFAULT_VALUES["render_resolution"])

        # Show a confirmation that the initial values were restored
        messagebox.showinfo("Defaults", "Default settings have been restored.")
    ...
    def handle_save(self) -> None:
        ...
        # Since the current values are stored in tk.StringVars, there's no need
        # to do anything, just set the 'previous' set of parameters to the current
        # values so the cancel button behaves as it should
        ...
        render_resolution_vars["previous"].set(render_resolution_vars["current"].get())

        # Close the modal
        self.destroy()


class _OutputFrame(ttk.Frame):
    ...
    def __init__(self, container: ttk.Frame) -> None:
        ...
        self.separate_output_directories_vars: OutputVars = {
            "default": tk.BooleanVar(
                value=DEFAULT_VALUES["separate_output_directories"],
            ),
            "current": tk.BooleanVar(
                value=CONFIG_VALUES["separate_output_directories"],
            ),
            "previous": tk.BooleanVar(
                value=CONFIG_VALUES["separate_output_directories"],
            ),
        }
        ...
        self.output_path = tk.StringVar(value=str(CONFIG_VALUES["render_output_path"]))
        ...
        browse_output_button = ttk.Button(
            self,
            text="Browse…",
            command=partial(_select_output, output_entry),
        )
        browse_output_button.grid(column=1, row=1)

        # Create the button to configure the render output settings and
        # add it to the grid
        output_settings_button = ttk.Button(self, text="⚙️", command=_ConfigOutputModal)
        output_settings_button.configure(width=3)
        output_settings_button.grid(column=2, row=1)


inputs_frame: _InputsFrame
...
class _MainFrame(ttk.Frame):
    ...
    def __init__(self, container: tk.Tk) -> None:
        ...
        global inputs_frame  # noqa: PLW0603
...
def init_gui(config_values: ConfigValues, default_values: ConfigValues) -> None:
    """Initialise the GUI for ArchSnap."""
    # Declare the config values and default values as global variables
    global CONFIG_VALUES, DEFAULT_VALUES  # noqa: PLW0603
    ...
    global main_window  # noqa: PLW0603
